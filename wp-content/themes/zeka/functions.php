...
function zeka_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'zeka_register_components', 20 );
Theme::load(
	array(
...
function zeka_assets() {
	return zeka_theme()->getAssetsManager();
}
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	zeka_assets()
		...
		->registerStylesheet( 'zeka-theme', '/theme/theme.css' );
    zeka_assets()->loadLocalGoogleFonts();
//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
//		zeka_assets()->addGoogleFont( $family, $variants );
//	}

	add_action( 'wp_enqueue_scripts', 'zeka_print_color_scheme', 0 );
} else {
	zeka_assets()
		->registerStyle( 'zeka-theme', zeka_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function zeka_add_woocommerce_suppoert() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'zeka_add_woocommerce_suppoert' );
...
function zeka_get_builder_plugin_slug() {
...
function zeka_theme_plugins( $plugins ) {

	$plugin_slug = zeka_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'zeka' ) : __( 'Kubio', 'zeka' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'zeka_theme_plugins' );

Hooks::prefixed_add_action(
	'after_plugin_activated',
	function ( $slug ) {

		if ( $slug === zeka_get_builder_plugin_slug() ) {
				...
				<div class="kubio-admin-row get-started-imported notice notice-success">
					<div>
						<p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'zeka' ),
									zeka_theme()->getName()
								)
							);
							?>
						...
						<p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'zeka' ),
									zeka_theme()->getName()
								)
							);
							?>
					...
					<div class="button imported-view-site-button">
						<a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html_e( 'View site', 'zeka' ); ?>
						</a>
...
function zeka_try_online_url() {
    return "https://kubiobuilder.com/go/try-theme/" . get_template();
}

add_filter('kubio/editor-try-online/url', 'zeka_try_online_url', 10);

function zeka_render_header_style() {

    zeka_theme()->get('css')->render();
}
add_action('wp_head', 'zeka_render_header_style', 100);
