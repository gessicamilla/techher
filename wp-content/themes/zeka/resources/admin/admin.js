...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".zeka-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.zeka_admin) {
    return;
  }

  var _window$zeka_admin = window.zeka_admin,
      builderStatusData = _window$zeka_admin.builderStatusData,
      getStartedData = _window$zeka_admin.getStartedData;
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".zeka-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.zeka-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".zeka-admin-big-notice").addClass("processing");
      $(".zeka-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".zeka-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.zeka-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.zeka-admin-big-notice').on("click", ".zeka-notice-dont-show-container", disableNotice);
  ...
  window.zeka_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
